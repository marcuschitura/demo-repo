...
//
 TEST_CASE("Word is not queryable if less than a specific size") {
   ...
   CHECK_FALSE(shortWord.isQueryable());
   CHECK(longWord.isQueryable());
...
// ------------- Tests for Paragraph ----------------
 ...
 // Create an empty Paragraph object
    ...
    // Create a Word object to search for
    Word search_word("test");
    
    // Call the contains method on the empty Paragraph
    auto [found, line_indices] = paragraph.contains(search_word);
    
    // Verify that the word was not found
    CHECK_FALSE(found);
...
//
 TEST_CASE("Word not present in Paragraph cannot be found") {
   ...
   // Create a Paragraph object
    ...
    paragraph.addLine(Line{"I do not know why this degree is so hard to get like wow."});

    // Search for a word that is not present
    auto [found, line_numbers] = paragraph.contains(Word{"Hello"});

    // Check that the word was not found
    CHECK_FALSE(found);
    
    // Check that the list of line numbers is empty
    CHECK(line_numbers.empty());

 }

 // cant make the rest work yes even the last one 
...
//// Integration test - both Paragraph and File Reader are tested together
 //TEST_CASE("File can be read into Paragraph and successfully searched") {
	// make sure that alice.txt is in the right location 
    // for this to work!
